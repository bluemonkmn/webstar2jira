...
# 7.30
$ENV{'SSDIR'}=$ssdb73;
MakeWorkspace('Migrate', $rootStream, $wsDir);
...
ReparentWorkspace('Migrate', '7.30');
RecursiveDelete();
VSSGet($ss73,'7.30A');
...
MakeStream('7.30L', '7.30L_SP');
MakeStream('7.30_WorkComplete', '7.30L');
ReparentWorkspace('Migrate', '7.30_WorkComplete');
RecursiveDelete();
VSSGetLatest($ss73);
CommitAll('Import latest FS 7.30 code after 7.30L release.');
 # 7.40
ReparentWorkspace('Migrate', $rootStream);
$ENV{'SSDIR'}=$ssdb74;
VSSWorkFold($ss74, $wsDir);
RecursiveDelete();
VSSGet($ss74, '7.40');
CommitAll('Import FS Label 7.40');
MakeSnapshot('7.40_GA', $rootStream);
MakeStream('7.40', '7.40_GA');
ReparentWorkspace('Migrate', '7.40');
RecursiveDelete();
VSSGet($ss74, '7.40A');
...
MakeStream('7.40K', '7.40K_SP');
MakeStream('7.40_WorkComplete', '7.40K');
ReparentWorkspace('Migrate', '7.40_WorkComplete');
RecursiveDelete();
VSSGetLatest($ss74);
CommitAll('Import latest FS 7.40 code after 7.40K release.');
# 7.50
MakeStream('7.50', $rootStream);
ReparentWorkspace('Migrate', '7.50', $wsDir);
$ENV{'SSDIR'}=$ssdb75;
